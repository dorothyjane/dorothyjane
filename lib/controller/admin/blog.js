...
exports.createBlog = function(req, res){
  ...
  var blog = {name : req.body.name}; //  double make sure they don't put crap in!!!
  connection.query("insert into blogs set ?", blog, function(err, result){
    if(err){
      return res.status(500).send("error setting blog");
    }
...
exports.updateBlog = function(req, res){
  connection.query('UPDATE blogs SET ? WHERE id = ?', [{ name: req.body.name }, req.params.id], function(err, response){
    if(err){
